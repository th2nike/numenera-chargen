...
fn test_batch_generation() {
    ...
    let characters: Vec<_> = (0..5).map(|_| generate_random(&data).unwrap()).collect();
...
fn test_edge_values_valid() {
    ...
    for _ in 0..10 {
        ...
        assert!(
            character.edge.might > 0 || character.edge.speed > 0 || character.edge.intellect > 0
        );
    }
}
