...
mod tests {
    ...
    fn test_all_types_valid() {
        ...
        for char_type in &data.types {
            ...
            assert!(
                !char_type.tagline.is_empty(),
                "Type tagline cannot be empty"
            );
            assert!(
                char_type.stat_pools.bonus_points > 0,
                "Bonus points must be positive"
            );
            assert!(
                !char_type.tier_abilities.is_empty(),
                "Type must have abilities"
            );
        }
    }
    ...
    fn test_all_descriptors_valid() {
        ...
        for descriptor in &data.descriptors {
            ...
            assert!(
                !descriptor.initial_links.is_empty(),
                "Descriptor must have initial links"
            );
        }
    }
    ...
    fn test_all_foci_valid() {
        ...
        for focus in &data.foci {
            ...
            assert!(
                !focus.suitable_types.is_empty(),
                "Focus must have suitable types"
            );
