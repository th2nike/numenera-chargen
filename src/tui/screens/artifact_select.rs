...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    let instructions = vec![
        Line::from(Span::styled(
            format!("Select up to {} artifacts (Space to toggle, R for random, C to clear)", max_artifacts),
    ...
    for (i, artifact) in app.game_data.artifacts.iter().enumerate() {
        ...
        lines.push(Line::from(Span::styled(
            format!("    {} | Depletion: {}", artifact.level_formula, artifact.depletion),
            Style::default().fg(Color::Gray),
        )));
        lines.push(Line::from(Span::styled(
            format!("    {}", truncate(&artifact.effect, 70)),
            Style::default().fg(Color::DarkGray),
        )));
